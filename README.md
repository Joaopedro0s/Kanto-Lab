...
| `monhunt.html`      | Tela do jogo MonHunt |
| `monlink.html` | Tela do jogo MonLink |
...
| `monhunt.js`        | Lógica do jogo MonHunt |
| `monlink.js`   | Lógica do jogo MonLink |
...
- No MonHunt, os dados **completos** de um Pokémon (tipo, altura, peso,
...
- No MonLink, a maioria das categorias usa listas prontas da API;
...
(MonHunt, Pokéle, etc.) e o mais fácil de reconhecer/buscar. Todo o
...
**MonHunt** — cada dificuldade define ao mesmo tempo: as gerações
...
**MonLink** — cada dificuldade define quantas vidas o jogador tem
...
- **Adicionar um atributo novo no MonHunt**: em `pokeapi.js`, acrescente
  a chave em `ATTRIBUTE_META`, adicione um `case` novo em
  `compareAttribute()`, e inclua a chave no array `attributes` da(s)
  dificuldade(s) desejada(s) em `DIFFICULTIES_MONHUNT`.
- **Adicionar uma categoria nova no MonLink**: em `monlink.js`,
  ...
  dificuldade em `DIFFICULTIES_MONLINK` (dentro de `pokeapi.js`).
- **Ajustar dificuldade**: os objetos `DIFFICULTIES_MONHUNT` e
  `DIFFICULTIES_MONLINK`, no topo de `pokeapi.js`, concentram
